...
impl<'a, E: bytemuck::Pod> Iter<'a, E> {
    pub(crate) fn new(tape: &'a FixedSizedTape<E>,
                      tapes_read_transaction: &'a TapesReadTransaction<'a>,
                      start_index: u64,    tape_len: u64,
    ) -> io::Result<Self> {
        const READ_AHEAD_SIZE: usize = 8 * 1024;

        let mut buf = vec![E::zeroed(); READ_AHEAD_SIZE / size_of::<E>()];

        let entries_to_read = buf.len() - (start_index as usize + buf.len()).saturating_sub(tape_len as usize);
...
impl<'a, E: bytemuck::Pod> Iterator for Iter<'a, E> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if index_to_buf == self.buf.len() - 1 {
            ...
            let entries_to_read = self.buf.len() - (offset as usize + self.buf.len()).saturating_sub(self.tape_len as usize);

            if entries_to_read != 0 {
                if let Err(e) = self.tapes_read_transaction.read_entries(self.tape, offset, &mut self.buf[..entries_to_read]) {
                    return Some(Err(e));
                }
            }
        }
