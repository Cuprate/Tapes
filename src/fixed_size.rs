use std::{cell::RefMut, cmp::max, io, marker::PhantomData, ops::Deref};

use bytemuck::Pod;

use crate::{memory::BackingMemory, unsafe_tape::UnsafeTape};

/// The full range of values in a fixed-sized tape.
pub struct FixedSizedTapeSlice<'a, T> {
    pub(crate) slice: &'a [T],
}

impl<T> Deref for FixedSizedTapeSlice<'_, T> {
    type Target = [T];
...
pub struct FixedSizedTapeAppender<'a, P: Pod, M: BackingMemory> {
    /// The backing tape file that all up-to-date readers see.
    pub(crate) backing_file: &'a UnsafeTape<M>,
    /// A mutable reference to a slot we can put a new instance of a tape in, if it needed a resize.
    pub(crate) resized_backing_file: RefMut<'a, Option<UnsafeTape<M>>>,
...
impl<P: Pod, M: BackingMemory> FixedSizedTapeAppender<'_, P, M> {
    /// Returns the backing tape handle that should be used for all operations in this writer
    fn backing_file(&self) -> &UnsafeTape<M> {
    ...
    /// Returns a [`FixedSizedTapeSlice`] which will include the entries witten by this writer.
    pub fn reader_slice(&mut self) -> io::Result<FixedSizedTapeSlice<'_, P>> {
        ...
        let bytes = unsafe {
            self.backing_file()
                .slice(0, self.current_used_bytes + *self.bytes_added)
        };

        Ok(FixedSizedTapeSlice {
...
pub struct FixedSizedTapePopper<'a, P: Pod, M: BackingMemory> {
...
impl<P: Pod, M: BackingMemory> FixedSizedTapePopper<'_, P, M> {
    /// Pop the entry in the tape, returning the index it was at.
    ...
    pub fn pop_entries(&mut self, amt: usize) {
        *self.current_used_bytes = self
            .current_used_bytes
            .checked_sub(amt * size_of::<P>())
            .unwrap();
    }
}
