...
pub struct BlobTapeAppender<'a, M: BackingMemory> {
...
impl<M: BackingMemory> BlobTapeAppender<'_, M> {
...
pub struct BlobTapePopper<'a, M: BackingMemory> {
    #[expect(dead_code)]
...
impl<M: BackingMemory> BlobTapePopper<'_, M> {
...
/// A full slice of bytes in a blob tape.
pub struct BlobTapeSlice<'a, M: BackingMemory> {
    #[expect(dead_code)]
    /// The backing tape (unused currently, but this type could access data in the future?)
    pub(crate) backing_file: &'a UnsafeTape<M>,
...
impl<M: BackingMemory> Deref for BlobTapeSlice<'_, M> {
    type Target = [u8];
